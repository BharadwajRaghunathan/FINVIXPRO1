...
def simulate_dashboard_data():
    ...
    for i in range(24):
        ...
        clicks_fluctuation = np.random.uniform(-0.2, 0.2)  # Add ±20% variation
        clicks = base_clicks * (1 + clicks_fluctuation)
        clicks = max(0, min(clicks, impressions))  # Ensure 0 <= clicks <= impressions
...
def upload_report():
    try:
        ...
        filename = f"upload_report_{model_type}_{timestamp}.pdf"
...
def download_results():
    try:
        ...
        base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'csv_excel_reports')
        sub_dir = model_type if model_type in ['roi', 'conversions'] else 'both'
        output_dir = os.path.join(base_dir, sub_dir)
        os.makedirs(output_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        if file_type == 'xlsx':
            filename = os.path.join(output_dir, f"{model_type}_results_{timestamp}.xlsx")
            df.to_excel(filename, index=False)
            mime_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        else:
            filename = os.path.join(output_dir, f"{model_type}_results_{timestamp}.csv")
